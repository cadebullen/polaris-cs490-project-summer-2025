              ...
              >
                ...
                {resumes.map((resume) => (
                  <option key={resume.resumeId} value={resume.resumeId}>
                    {resume.customName || resume.fileName || `Resume ${resume.resumeId.slice(0, 8)}`}
                  </option>
                ))}
              </select>
            )}
            ...
            {selectedResume && (
              <div className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30 rounded-lg border border-green-200 dark:border-green-700">
                <h4 className="font-semibold text-green-700 dark:text-green-300 mb-2">
                  {selectedResume.customName || selectedResume.fileName || "Selected Resume"}
                </h4>
                <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                  <p><strong>Resume ID:</strong> {selectedResume.resumeId.slice(0, 12)}...</p>
                  {selectedResume.objective && (
                    <p><strong>Objective:</strong> {selectedResume.objective.slice(0, 100)}...</p>
                  )}
                  {selectedResume.createdAt && (
                    <p><strong>Created:</strong> {new Date(selectedResume.createdAt).toLocaleDateString()}</p>
                  )}
                </div>
              </div>
            )}
